...
// adjust servo position according to t he USS read value

  // add your code here!
   if (dist_raw<=180)
     myservo.writeMicroseconds(_DUTY_MIN);
   if (180<dist_raw && dist_raw<=360)
     myservo.writeMicroseconds((dist_raw)*((_DUTY_MAX-_DUTY_MIN)/(360-180))-1293);
   if (360<dist_raw)
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading == 0.0) reading = dist_raw;
